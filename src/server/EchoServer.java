    ...
    public void handleCmdFromClient(String cmd, ConnectionToClient client) {
        if (!cmd.contains("login") && client.getInfo("id") == null) { //If the client has not connected yet and the command is not a login
            ...
            if (cmd.equals("logoff")) {
                try {
                    client.close();
                    comm.sendToAllClients(client.getInfo("id") + " has disconnect.");
                ...
                if (client.getInfo("id") == null) {
                    try {
                        ...
                        serverUI.display("Client " + client.getInfo("id") + " has connect.");
                        comm.sendToAllClients(client.getInfo("id") + " has connect.");
                        String allClients = getAllLogins();
                        comm.sendToAllClients("#connectedClients " + allClients);
                    } catch (Exception e) {
